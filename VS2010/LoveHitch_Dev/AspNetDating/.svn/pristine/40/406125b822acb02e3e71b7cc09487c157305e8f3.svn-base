...
namespace AspNetDating.Classes
{
    public class FacebookInvitation
    {
        ...
        public static int[] Search(string senderId, string recipientId, DateTime fromTime)
        {
            using (SqlConnection conn = Config.DB.Open())
            {
                SqlDataReader reader =
                    SqlHelper.ExecuteReader(conn, "SearchFacebookInvitations",
                                            senderId,
                                            recipientId,
                                            fromTime);
        ...
        public static FacebookInvitation Fetch(int id)
        {
            ...
            {
                SqlDataReader reader =
                    SqlHelper.ExecuteReader(conn, "LoadFacebookInvitation", id);
        ...
        public void Save()
        {
            using (SqlConnection conn = Config.DB.Open())
            {
                object result = SqlHelper.ExecuteScalar(conn, "SaveFacebookInvitation", id,
