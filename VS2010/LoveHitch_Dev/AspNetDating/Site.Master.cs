        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!(Page is ShowStatus || IsPostBack))
            {
                if (Request.QueryString["InvitedBy"] != null)
                    Session["InvitedBy"] = Request.QueryString["InvitedBy"];

                Session["CurrentRawUrl"] = Request.Url.Authority + Request.RawUrl;
                Session["CurrentPage"] = Request.Url.Authority + Request.Url.LocalPath;

                if (Request.UrlReferrer != null && Request.Url.LocalPath != Request.UrlReferrer.LocalPath)
                    Session["RefererPage"] = Request.UrlReferrer.Authority + Request.UrlReferrer.LocalPath;
            }
            ...
            */
            ...
            if (Context.Session["LanguageId"] == null)
                Context.Session["LanguageId"] = PageBase.GetLanguageId();

            bool isRtl = Language.Fetch(Convert.ToInt32(Context.Session["LanguageId"])).Direction == "rtl";
            Session["Direction"] = isRtl ? "rtl" : "ltr";
