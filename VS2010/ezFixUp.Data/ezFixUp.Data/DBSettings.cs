...
namespace ezFixUp.Classes
{
    public class DBSettings
    {
        ...
        private static string Get(string Key)
        {
            ...
            {
                string value = (string) SqlHelper.GetDB().ExecuteScalar("FetchSetting", Key);

                if (HttpContext.Current != null && Key != null && value != null)
                {
                    //Global.AddCacheItem("Settings", "DBSettings_" + Key, value);
                    HttpContext.Current.Cache.Insert("DBSettings_" + Key,value, null, DateTime.Now.AddHours(1),Cache.NoSlidingExpiration);
                }
                return value;
            }
        }
        ...
        public static void Set(string Key, string Value)
        {
            ...
            using (SqlConnection conn = Config.DB.Open())
            {
                SqlHelper.GetDB().ExecuteNonQuery( "UpdateSetting", Key, Value);
            }
