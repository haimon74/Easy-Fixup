...
namespace ezFixUp.Classes
{
    public class EcardCategory
    {
        ...
        private static EcardCategory[] Fetch(int? id, int? parentID, string title, eSortColumn sortColumn)
        {
            ...
            {
                SqlDataReader reader = (SqlDataReader) SqlHelper.GetDB().ExecuteReader( FetchProcedureName, id, parentID, title, sortColumn);
        ...
        public void Save()
        {
            using (SqlConnection conn = Config.DB.Open())
            {
                object result = SqlHelper.GetDB().ExecuteScalar( GetSaveStoreProceduresName(), _id, _parentID, _isActive, _title);
        ...
        public static void Delete(int id, string procedureName)
        {
            using (SqlConnection conn = Config.DB.Open())
            {
                SqlHelper.GetDB().ExecuteNonQuery( procedureName, id);
            }
        }
