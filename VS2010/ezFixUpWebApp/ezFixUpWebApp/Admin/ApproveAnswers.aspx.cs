...
namespace ezFixUp.Admin
{
    ...
    public partial class ApproveAnswers : AdminPageBase
    {
        ...
        {
            this.dgPendingApproval.ItemCommand += new DataGridCommandEventHandler(dgPendingApproval_ItemCommand);
            this.dgPendingApproval.ItemCreated += new DataGridItemEventHandler(dgPendingApproval_ItemCreated);
            this.dgPendingApproval.ItemDataBound += new DataGridItemEventHandler(dgPendingApproval_ItemDataBound);
            this.dgPendingApproval.PageIndexChanged +=
                new DataGridPageChangedEventHandler(dgPendingApproval_PageIndexChanged);
            this.dropAnswersPerPage.SelectedIndexChanged += new EventHandler(dropAnswersPerPage_SelectedIndexChanged);
        }
        ...
        private void dgPendingApproval_ItemCreated(object sender, DataGridItemEventArgs e)
        {
        ...
        private void dgPendingApproval_PageIndexChanged(object source, DataGridPageChangedEventArgs e)
        {
            dgPendingApproval.CurrentPageIndex = e.NewPageIndex;
            PopulateDataGrid();
        }

        private void dropAnswersPerPage_SelectedIndexChanged(object sender, EventArgs e)
        {
            dgPendingApproval.PageSize = Convert.ToInt32(dropAnswersPerPage.SelectedValue);
            dgPendingApproval.CurrentPageIndex = 0;
            PopulateDataGrid();
        ...
        private void dgPendingApproval_ItemDataBound(object sender, DataGridItemEventArgs e)
        {
            if (e.Item.ItemIndex == -1)
                return;
