...
namespace ezFixUp.Classes
{
    ...
    public class Admin
    {
        ...
        public class AdminPrivileges
        {
            ...
            [Reflection.DescriptionAttribute("Manage FAQs")]
            public eAccess FAQs = eAccess.None;

            [Reflection.DescriptionAttribute("Manage ask about message questions")]
            public eAccess AskAboutQuestions = eAccess.None;
            ...
            [Reflection.DescriptionAttribute("Edit Coupons")]
            public eAccess editCoupons = eAccess.None;
        ...
        protected void updateLastLogin(string sessionID)
        {
            ...
            {
                SqlHelper.GetDB().ExecuteNonQuery( "UpdateAdminLastLogin", Username, sessionID);
            }
        }
        ...
        public void Load()
        {
            //using (var conn = Config.DB.Open())
            {
                using (var reader = SqlHelper.GetDB().ExecuteReader( "LoadAdmin", username))
                {
        ...
        private static Admin[] FetchAdmins(string username)
        {
            //using (var conn = Config.DB.Open())
            {
                ...
                if (string.IsNullOrWhiteSpace(username))
                    reader = SqlHelper.GetDB().ExecuteReader("FetchAdmins");
                else
                    reader = SqlHelper.GetDB().ExecuteReader("FetchAdmins", username);
        ...
        public static void Delete(string username)
        {
            //using (var conn = Config.DB.Open())
            {
                SqlHelper.GetDB().ExecuteNonQuery( "DeleteAdmin", username);
            }
        }
