    ...
    public class EcardType
    {
        ...
        private static EcardType[] Fetch(int? id, int? categoryId, string name, eType? type, bool? active, eSortColumn sortColumn)
        {
            //using (var conn = Config.DB.Open())
            {
                ...
                using (var reader = SqlHelper.GetDB().ExecuteReader("FetchEcardTypes",
                                                                    id,
                                                                    categoryId,
                                                                    name,
                                                                    (int?) type,
        ...
        public void Save()
        {
            //using (var conn = Config.DB.Open())
            {
                object result = SqlHelper.GetDB().ExecuteScalar( "SaveEcardType",
                                                        id,
                                                        categoryId,
                                                        name,
                                                        phrase,
        ...
        public static void Delete(int id)
        {
            //using (var conn = Config.DB.Open())
            {
                SqlHelper.GetDB().ExecuteNonQuery( "DeleteEcardType", id);
            }
        }
        ...
        public static byte[] LoadContent(int ecardTypeID)
        {
            ...
            {
                using (var reader =
                    SqlHelper.GetDB().ExecuteReader("FetchEcardTypeContent", ecardTypeID))
                {
