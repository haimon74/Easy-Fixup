...
namespace ezFixUp.Classes
{
    ...
    public class Group
    {
                                    ...
                                    int? activeMembers, bool? autojoin, int? numberOfGroups, eSortColumn sortColumn)
        {
            ...
            //{
            //    var reader = SqlHelper.GetDB().ExecuteReader( "FetchGroups", id, name, description, dateCreated,
                                    ...
                                    string keyword, bool searchInDescription, int? numberOfGroups, eSortColumn sortColumn)
        {
            ...
            ////using (var conn = Config.DB.Open())
            //{
            //    var reader = SqlHelper.GetDB().ExecuteReader( "SearchGroups",
        ...
        private static Group[] FetchGroupsBy(int? categoryID, string username, bool? approved)
        {
            //using (var conn = Config.DB.Open())
            {
                ...
                if (categoryID != null)
                {
                    reader = SqlHelper.GetDB().ExecuteReader("FetchGroupsByCategory", categoryID, approved);
                }
                else
                {
                    reader = SqlHelper.GetDB().ExecuteReader("FetchGroupsByUsername", username, approved);
                }
        ...
        private static int[] SearchGroupsByUsername(string username, bool? approved, eSortColumn eSortColumn)
        {
            //using (var conn = Config.DB.Open())
            {
                ...
                using (var reader = SqlHelper.GetDB().ExecuteReader("SearchGroupsByUsername", 
                    username, approved, eSortColumn))
                {
        ...
        public static Image LoadIcon(int groupID)
        {
            //using (var conn = Config.DB.Open())
            {
                using (var reader = SqlHelper.GetDB().ExecuteReader("FetchGroupIcon", groupID))
                {
    ...
    public class BasicSearchGroup
    {
        ...
        public GroupSearchResults GetResults()
        {
            //using (var conn = Config.DB.Open())
            {
                ...
                using (var reader = SqlHelper.GetDB().ExecuteReader("BasicSearchGroup",
