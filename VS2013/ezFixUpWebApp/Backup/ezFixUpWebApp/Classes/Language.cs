...
namespace ezFixUp.Classes {
	...
	public static class Lang {
		...
		private static string Trans(string key, bool adminPanel) {
			...
			if(context == null || context.Session == null || Global.GetSessionState()["LanguageId"] == null) {
				return Trans(Config.Misc.DefaultLanguageId, key, adminPanel);
			}

			try {
				int languageId = Convert.ToInt32(Global.GetSessionState()["LanguageId"]);
	...
	public class Language {
		...
		public static Language[] FetchAll() {
			...
			//using (var conn = Config.DB.Open()) 
            {
                ...
                using (var reader = SqlHelper.GetDB().ExecuteReader("FetchLanguages"))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        language.browserLanguages = reader["BrowserLanguages"] != DBNull.Value
                                                        ...
                                                        : null;
                        language.ipCountries = reader["IpCountries"] != DBNull.Value
                                                   ? (string) reader["IpCountries"]
                                                   : null;
		...
		private static Language[] Fetch(int id, string name) {
			...
			//using (var conn = Config.DB.Open()) 
            {
                using (var reader = SqlHelper.GetDB().ExecuteReader("FetchLanguage", id, name))
                {
	...
	public class Translation {
		...
		public static string[] FetchTranslationKeys(bool adminPanel) {
			...
			//using (var conn = Config.DB.Open()) 
            {
                using (var reader = SqlHelper.GetDB().ExecuteReader("FetchTranslationKeys",
                                    (int)(adminPanel ? TranslationType.Admin : TranslationType.User))) 
                {
                    while (reader.Read())
                    {
