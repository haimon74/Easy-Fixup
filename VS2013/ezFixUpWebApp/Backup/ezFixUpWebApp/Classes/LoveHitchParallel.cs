...
namespace ezFixUp.Classes.ezFixUpParallel
{
    ...
    public class LoadUserResultThreadClass : IWorkThreadClass
    {
        ...
        public virtual void DoWorkThread(object parameter)
        {
            ...
            var user = Classes.User.Load(key);
            if (user != null)
                lock (outValues)
                {
                    outValues.Add(user);
    ...
    public class ezFixUpAsyncResult : IAsyncResult
        {
            ...
            public ezFixUpAsyncResult(AsyncCallback cb, object state)
            {
                _cb = cb;
                _state = state;
