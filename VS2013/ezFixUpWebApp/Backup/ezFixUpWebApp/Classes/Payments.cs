...
namespace ezFixUp.Classes
{
    public class Payments
    {
                                              ...
                                              string notes, int status)
        {
            ...
            {
                object result = SqlHelper.GetDB().ExecuteScalar( "SavePaymentHistory", username, paymentprocessor, amount, description,
                                          notes, status);
                ...
                try
                {
                    ...
                    else if (cells[0].Trim('"') == "REBILL")
                    {
                        ...
                        if (subscription != null)
                        {
                            ...
                            try
                            {
                                var user = Classes.User.Load(subscription.Username);
