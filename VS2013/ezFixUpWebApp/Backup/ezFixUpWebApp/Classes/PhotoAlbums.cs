    ...
    public class PhotoAlbum
    {
        ...
        private static PhotoAlbum[] Fetch(int? id, string username, string name, eAccess? access)
        {
            //using (var conn = Config.DB.Open())
            {
                ...
                using (var reader = SqlHelper.GetDB().ExecuteReader("FetchPhotoAlbums",
                                                                    id, username, name, access))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        photoAlbum.coverPhotoID = reader["CoverPhotoID"] != DBNull.Value
                                                      ? (int?) reader["CoverPhotoID"]
                                                      : null;
        ...
        public void Save()
        {
            //using (var conn = Config.DB.Open())
            {
                object result = SqlHelper.GetDB().ExecuteScalar( "SavePhotoAlbum",
        ...
        public static void Delete(int id)
        {
            //using (var conn = Config.DB.Open())
            {
                SqlHelper.GetDB().ExecuteNonQuery( "DeletePhotoAlbum", id);
            }
