...
namespace ezFixUp.Classes
{
    ...
    public class WebPartUserControl : UserControl, IWebPart, IWebEditable, IWebActionable
    {
        ...
        private void OnCustomEditVerbClick(object sender,
                                           WebPartEventArgs e)
        {
            var manager = (CustomWebPartManager) 
            ...
            foreach (WebPart part in e.WebPart.Zone.WebParts)
            {
                if (part != e.WebPart)
                {
                    ((CustomWebPartZone) e.WebPart.Zone).CloseWebPart(part);
                }
            }
        }
    ...
    public class CustomWebPartManager : WebPartManager
    {
        ...
        {
            get { return (Global.GetSessionState().ContainsKey("SortedWebPartIDs")) 
                ? Global.GetSessionState()["SortedWebPartIDs"] as List<string> : null; }
            set { Global.GetSessionState()["SortedWebPartIDs"] = value; }
        }
        ...
        public void AddWebPartUserControl(WebPartInfo info, CustomWebPartZone zone, int zoneIndex)
        {
    ...
    public class CustomWebPartZone : System.Web.UI.WebControls.WebParts.WebPartZone
    {
    ...
    public class CustomEditorZone : EditorZone
    {
        protected override void OnSelectedWebPartChanged(object sender, WebPartEventArgs e)
        {
            ...
            {
                ((CustomWebPartManager) WebPartManager).ReOpenClosedWebParts();
