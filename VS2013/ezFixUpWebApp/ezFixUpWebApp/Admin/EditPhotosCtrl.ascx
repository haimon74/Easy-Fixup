<%@ Control Language="c#" AutoEventWireup="True" Codebehind="EditPhotosCtrl.ascx.cs" Inherits="ezFixUp.Admin.EditPhotosCtrl" TargetSchema="http://schemas.microsoft.com/intellisense/ie5"%>
			...
			<table cellSpacing="0" cellPadding="0" id="stable" border="0">
			    ...
			    <tr>
					...
					<!--<asp:Label ID="lblError" ForeColor="Red" EnableViewState="False" Font-Bold="True" Runat="server" />-->
					    ...
					    <asp:LinkButton id="btnUpload" CommandName="UploadPhoto" CommandArgument='<%# ((string)(Eval("PhotoId")) == "0")?NewTempID:Eval("PhotoId") %>' Runat="server" Text = '<%# ezFixUp.Classes.Lang.TransA("Upload")%>'>
					    </asp:LinkButton>
		        ...
		        <tr>
					<td class="editphotos-links" colspan="2">
						<asp:CheckBox ID="chkExplicitPhoto" Runat="server" Text='<%# ezFixUp.Classes.Lang.TransA("Explicit Photo")%>' Checked='<%# Eval("ExplicitPhoto") %>' Visible='<%# (ezFixUp.Classes.Config.Photos.EnableExplicitPhotos && ((string)Eval("PhotoId") != "0"))%>'></asp:CheckBox>
					    <%#  (ezFixUp.Classes.Config.Photos.EnableExplicitPhotos && ((string)Eval("PhotoId") != "0"))?"|":""%>					
