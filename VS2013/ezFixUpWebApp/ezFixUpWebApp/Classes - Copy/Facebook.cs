...
namespace ezFixUp.Data
{
    ...
    [Serializable]
    public class LoveHitchFacebookApp : FacebookApp, ISerializable
    {
        public LoveHitchFacebookApp()
            : base()
        {
        }
        public LoveHitchFacebookApp(string accessToken)
            : base(accessToken)
        {
        }
        public LoveHitchFacebookApp(IFacebookSettings settings)
            : base(settings)
        {
        }
        public LoveHitchFacebookApp(SerializationInfo info, StreamingContext context)
        {
    ...
    public class FacebookHelper
    {
        ...
        public class AddNewFacebookFriendClass
        {
            public void AddNewFacebookFriend(object fbFriendData)
            {
                ...
                try
                {
                    ...
                    //var cityName = Config.Users.MapFacebookCityToLoveHitchCity( HttpContext.Current,
                    //                                                            fbUser.Location,
                    //                                                            out regionName,
                    //                                                            out countryName);
                    //if (cityName.IsNotNullOrEmpty())
                    //    cityName = Config.Users.MapFacebookCityToLoveHitchCity( HttpContext.Current,
                    //                                                            fbUser.HomeTown,
                    //                                                            out regionName,
                    //                                                            out countryName);
                    var newUser = new User(friendId.ToString())
                                      {
                                          ...
                                          FacebookID = friendId,
                                          Password = pass,
                                          //InterestedIn =  User.eGender.Matchmaker,
                                          Birthdate = fbUser.Birthdate,
                                          Email = String.Format("{0}@lovehitch.com", fbUser.Username),
                                          Name = fbUser.Name,
                                          ProfileVisible = true,
                    ...
                    User.eGender? interestedIn = null;
                    switch (fbUser.GenderStr.ToLower(CultureInfo.InvariantCulture))
                    {
                        case "male":
                            gender = User.eGender.Male;
                            interestedIn = User.eGender.Female;
                            break;
                        case "female":
                            gender = User.eGender.Female;
                            interestedIn = User.eGender.Male;
                            break;
                        default:
                            gender = User.eGender.Matchmaker;
                            interestedIn = User.eGender.Matchmaker;
                            break;
                    }
        ...
        public class FetchAndSaveSaveFacebookFriendImageClass
        {
            ...
            {
                string imgUrl = ImageHelper.CreateFacebookImageUrl((long)facebookId,
                                                                        ImageHelper.eFacebookImageType.Large);
            ...
            public void FetchAndSaveFacebookImageBySource(string imgUrl, long facebookId)
            {
                ...
                try
                {
                    ...
                    User user = User.Load(facebookId.ToString());
        ...
        public static void AddFriends(User user, int maxFriendsToAdd, HttpContext context, LoveHitchFacebookApp facebook)
        {
            if (facebook == null)
            ...
            try
            {
                ...
                if (context.Session != null)
                {
                    if (friendsData.Count > 60)
                    {
                        if (context.Session["SessionRefreshState"] == null)
                        {
                            context.Session["SessionRefreshState"] =
                                SessionRefreshStateEnum.AddFriendsWithMutualFriends;
                    ...
                    {
                        context.Session["SessionRefreshState"] =
                            SessionRefreshStateEnum.CacheNormalImagesOfFriendsWithMutualFriends;
                    }
                }
            ...
            try
            {
                if (lstFriendsIds != null && lstFriendsIds.Any())
                {
                    ...
                    var parameterizedThreadsList =
                        new LoveHitchParallel.ParameterizedThreadsList<FacebookFriendData>(fbFriendsDataList,
            ...
            finally
            {
                ...
                //var parameterizedThreadsList =
                //    new LoveHitchParallel.ParameterizedThreadsList<string>(ftu.Usernames, ftu.TakenUsernames, ftu.AddUsernameIfTaken, context);
                //parameterizedThreadsList.RunAsBackgroundThreads = true;
                //parameterizedThreadsList.ExecuteParallelWork(maxThreadsInPool);
                ...
                if (lstAddUsernamesToFriends.Any())
                {
                    var addFriendsDataThreadsList =
                        new LoveHitchParallel.ParameterizedThreadsList<FacebookFriendDataClass>(
                ...
                var dic = (Dictionary<string, LovehitchBackgroundWorker>)Global.AppBackgroundWorkersDic;
                if (context.Session != null && CurrentSessionUserID > 0)
                {
                    if (dic != null)
                    {
                        ...
                        //       dic["ezFixUp.Data.AddNewFacebookFriendsBackgroundWorker"];
                ...
                //                        dic["ezFixUp.Data.LoadAndSaveFacebookImageBackgroundWorker"];
        ...
        public class FacebookFriendDataClass : LoveHitchParallel.WorkThreadClass
        {
            public string ViewerUsernme;
            public string FriendUsernme;
            public List<SearchMutualFriendsClass> MutualFriendsList;
            public LoveHitchFacebookApp FacebookAppSession;
            ...
            public override void DoWorkThread(object parameter)
            {
                ...
                try
                {
                    ...
                    using (User user = User.Load(frData.ViewerUsernme))
                    {
                        if (!user.IsUserInFriendList(frData.FriendUsernme))
                            user.AddToFriends(frData.FriendUsernme, true);
                    }
                    using (User friend = User.Load(frData.FriendUsernme))
                    {
        ...
        {
            get { return (long)(HttpContext.Current.Session["CurrentFacebookID"] ?? 0); }
            set { HttpContext.Current.Session["CurrentFacebookID"] = value; }
        }
        public static bool IsCurrentSessionConnected
        {
            get { return (bool)(HttpContext.Current.Session["IsFacebookConnected"] ?? false); }
            set { HttpContext.Current.Session["IsFacebookConnected"] = value; }
        }
        public static int SessionConnectTriesElapsed
        {
            get { return int.Parse((string)(HttpContext.Current.Session["FacebookConnectTriesElapsed"] ?? "0")); }
            set { HttpContext.Current.Session["FacebookConnectTriesElapsed"] = value.ToString(); }
        }
        public static string CurrentSessionAccessToken
        {
            get { return HttpContext.Current != null ? (string)HttpContext.Current.Session["facebookAccessToken"] : null; }
        ...
        public static LoveHitchFacebookApp GetSessionFacebookApp(bool redirectToLogin)
        {
            LoveHitchFacebookApp facebook = GetSessionFacebookApp();
        ...
        public static LoveHitchFacebookApp SessionFacebookApp
        {
            get { return GetSessionFacebookApp(); }
            set { if (HttpContext.Current != null) HttpContext.Current.Session["facebook"] = value; }
        }

        public static LoveHitchFacebookApp GetSessionFacebookApp()
        {
            LoveHitchFacebookApp facebook = null;
            if (HttpContext.Current != null && HttpContext.Current.Session != null && HttpContext.Current.Session["facebook"] != null)
            {
                facebook = (LoveHitchFacebookApp)HttpContext.Current.Session["facebook"];
            }
            else if (CurrentSessionAccessToken.IsNotNullOrEmpty())
            {
                facebook = new LoveHitchFacebookApp(CurrentSessionAccessToken);
            }
            ...
            {
                facebook = new LoveHitchFacebookApp(HttpContext.Current.Request.QueryString["access_token"]);
            }
            else
            {
        ...
        {
            User user = null;
            string[] usernames = User.FetchUsernamesByFacebookID(new[] { (long)(id.HasValue ? id : 0) });
                ...
                if (fbUser != null)
                {
                    ...
                    if (fbUser.ContainsKey("gender"))
                        if ((string)fbUser["gender"] == "male")
                            ...
                            user.Gender = User.eGender.Female;
                    ...
                    if (fbUser.ContainsKey("interested_in"))
                    {
                        ...
                        if (interestedIn == "male")
                            user.InterestedIn = User.eGender.Male;
                        else if (interestedIn == "female")
                            user.InterestedIn = User.eGender.Female;
                    }
                    ...
                    if (fbUser.ContainsKey("location"))
                    {
                        ...
                        if (location.ContainsKey("name"))
                        {
                            ...
                            user.City = Config.Users.MapFacebookCityToLoveHitchCity(HttpContext.Current,
        ...
        public static JsonArray GetFqlResult(LoveHitchFacebookApp facebook, string qry)
        {
            try
            {
            ...
            {
                ExceptionLogger.Log("public static JsonArray GetFqlResult(LoveHitchFacebookApp facebook, string qry)", ex);
                return null;
            }
        }
        public static void GetAsyncFqlResult(LoveHitchFacebookApp facebook,
        ...
        {
            LoveHitchFacebookApp fb = GetSessionFacebookApp();
            return GetFriendsDataList(context, fb);
        }
        public static IList<FacebookFriendData> GetFriendsDataList(HttpContext context, LoveHitchFacebookApp fb)
        {
        ...
        private static FriendsData GetFriendsDataAsyncQueries(LoveHitchFacebookApp fb, long id)
        {
        ...
        public static IList<FacebookFriendData> GetFriendsDataList(HttpContext context, LoveHitchFacebookApp fb, long fbUserId)
        {
                ...
                try
                {
                    ...
                    if (o.ContainsKey("current_location"))
                    {
                        ...
                        if (location != null && location.ContainsKey("name"))
                        {
                            ...
                            //locationCity = Config.Users.MapFacebookCityToLoveHitchCity(context, locationName, out locationRegion, out locationCountry);
                        }
                    }
                    ...
                    if (o.ContainsKey("hometown_location"))
                    {
                        ...
                        if (hometown != null && hometown.ContainsKey("name"))
                        {
                            ...
                            //hometownCity = Config.Users.MapFacebookCityToLoveHitchCity(context, hometownName, out hometownRegion, out hometownCountry);
                        }
                    }
                        ...
                        new FacebookFriendData
                            {
                                ...
                                PicUrl =
                                    (string)
                                    (o["pic"] ??
                                     ImageHelper.CreateFacebookImageUrl(long.Parse((string)o["uid"]),
                                                                         ImageHelper.eFacebookImageType.Large)),
        ...
        public static IList<FacebookFriendData> GetFriendsDataList(LoveHitchFacebookApp fb, long fbUserId)
        {
                            ...
                            new FacebookFriendData
                                {
                                    ...
                                    PicUrl =
                                        (string)
                                        (o["pic"] ??
                                         ImageHelper.CreateFacebookImageUrl((long)o["uid"],
                                                                             ImageHelper.eFacebookImageType.Large)),
                                    //Status = (string)o["relationship_status"],
        ...
        {
            public MutualFriendsCountCallbackClass(LoveHitchFacebookApp facebookApp)
            {
                ItsQueryCounter = 0;
            ...
            public LoveHitchFacebookApp ItsFacebookApp;
        ...
        {
            public FriendsDataCallbackClass(LoveHitchFacebookApp facebookApp)
            {
                ItsQueryCounter = 0;
                ItsFacebookApp = facebookApp;
                ItsSyncObject = "";
        ...
        public static string GetProfilePictureSource(LoveHitchFacebookApp facebook, long fbUserId)
        {
            string source = String.Format("https://graph.facebook.com/{0}/picture?type={1}",
                                              fbUserId, ImageHelper.eFacebookImageType.Large);
        ...
        public static JsonArray GetFriendsData(LoveHitchFacebookApp fb, long fbUserId)
        {
        ...
        public static void LoginLovehitchAppThroughFacebook()
        {
            if (Config.Misc.EnableFacebookIntegration)
            {
                ...
                else
                {
                    ...
                    string[] usernames = User.FetchUsernamesByFacebookID(new[] { id });
    ...
    public static class FacebookHelperExtensions
    {
        ...
        public static LoveHitchFacebookApp GetSessionFacebookApp(this HttpContext context, bool redirectToLogin)
        {
            LoveHitchFacebookApp facebook = context.GetSessionFacebookApp();
        ...
        public static void SetSessionFacebookApp(this HttpContext context, LoveHitchFacebookApp facebookApp)
        {
            lock (context)
            {
        ...
        public static LoveHitchFacebookApp GetSessionFacebookApp(this HttpContext context)
        {
            LoveHitchFacebookApp facebook = null;
            ...
            lock (context)
            {
                ...
                {
                    facebook = (LoveHitchFacebookApp)context.GetSession()["facebook"];
                }
                else if (context.GetCurrentSessionAccessToken().IsNotNullOrEmpty())
                {
                    facebook = new LoveHitchFacebookApp(context.GetCurrentSessionAccessToken());
                }
                else if (context.Request.QueryString["access_token"] != null &&
                         context.Request.QueryString["access_token"].Length > 0)
                {
                    facebook = new LoveHitchFacebookApp(context.Request.QueryString["access_token"]);
                }
                else
                {
                    facebook = new LoveHitchFacebookApp();
        ...
        public static JsonArray GetFqlResult(this LoveHitchFacebookApp facebook, string qry)
        {
            JsonArray result = (JsonArray)facebook.Query(qry);
            return result;
        }
