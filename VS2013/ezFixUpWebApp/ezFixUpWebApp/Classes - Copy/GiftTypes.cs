...
namespace ezFixUp.Data
{
    public class GiftType
    {
        ...
        private static GiftType[] Fetch(int? id, int? categoryId, int? price, string name, bool? active, eSortColumn sortColumn)
        {
            ...
            {
                SqlDataReader reader = (SqlDataReader) SqlHelper.GetDB().ExecuteReader( "FetchGiftTypes",
                                                               id,
                                                               categoryId,
                                                               price,
                                                               name,
        ...
        public void Save()
        {
            using (SqlConnection conn = Config.DB.Open())
            {
                object result = SqlHelper.GetDB().ExecuteScalar( "SaveGiftType",
                                                        id,
                                                        CategoryId,
                                                        Name,
                                                        Phrase,
        ...
        public static void Delete(int id)
        {
            using (SqlConnection conn = Config.DB.Open())
            {
                SqlHelper.GetDB().ExecuteNonQuery( "DeleteGiftType", id);
            }
        }
        ...
        public static byte[] LoadContent(int giftTypeID)
        {
            using (SqlConnection conn = Config.DB.Open())
            {
                SqlDataReader reader = (SqlDataReader) 
                    SqlHelper.GetDB().ExecuteReader( "FetchGiftTypeContent", giftTypeID);
