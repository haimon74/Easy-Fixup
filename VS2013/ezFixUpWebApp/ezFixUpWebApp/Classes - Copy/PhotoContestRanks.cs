...
namespace ezFixUp.Data
{
    public class PhotoContestRank
    {
        ...
        public static PhotoContestRank[] Load(string username, int? contestId, int? entryId, int? value)
        {
            ...
            {
                SqlDataReader reader = (SqlDataReader) SqlHelper.GetDB().ExecuteReader( "LoadPhotoContestRanks", username,
        ...
        public void Save()
        {
            using (SqlConnection conn = Config.DB.Open())
            {
                SqlHelper.GetDB().ExecuteNonQuery( "SavePhotoContestRank", username, contestId,
                    entryId, value);
            }
        }
    }
