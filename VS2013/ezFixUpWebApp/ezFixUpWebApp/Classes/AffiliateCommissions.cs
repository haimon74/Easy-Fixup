    ...
    public class AffiliateCommission
    {
        ...
        private static AffiliateCommission[] Fetch(int? id, int? affiliateID, int? paymentHistoryID, string username, decimal? amount, DateTime? timeStamp, eSortColumn sortColumn)
        {
            //using (var conn = Config.DB.Open())
            {
                ...
                using (var reader = SqlHelper.GetDB().ExecuteReader("FetchAffiliateCommissions",
                                                                    id, affiliateID, paymentHistoryID, username, amount,
                                                                    timeStamp, sortColumn))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        affiliateCommission.paymentHistoryID = reader["PaymentHistoryID"] == DBNull.Value
                                                                   ? null
                                                                   : (int?) reader["PaymentHistoryID"];
        ...
        public void Save()
        {
            ...
            {
                object result =
                    SqlHelper.GetDB().ExecuteScalar( "SaveAffiliateCommission", id, affiliateID, paymentHistoryID, username, amount,
                                            timeStamp, notes);
        ...
        private static void DeleteBy(int? id, int? affiliateID, int? paymentHistoryID, string username)
        {
            //using (var conn = Config.DB.Open())
            {
                SqlHelper.GetDB().ExecuteNonQuery( "DeleteAffiliateCommission", id, affiliateID, paymentHistoryID, username);
            }
        }
        ...
        public static void ApplyCommission(string username, int? paymentHistoryID, decimal amount, string notes)
        {
            ...
            if (user!= null && user.AffiliateID != null)
            {
                ...
                if (affiliate != null)
                {
                    ...
                    //using (var conn = Config.DB.Open())
                    {
                        SqlHelper.GetDB().ExecuteNonQuery( "ApplyAffiliateCommission", affiliate.ID, paymentHistoryID,
