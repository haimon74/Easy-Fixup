...
namespace ezFixUp.Classes
{
    public class SavedSearch
    {
        ...
        public static SavedSearch Create(string username, string name, Classes.User.eGender gender,
        ...
        public void Save()
        {
            //using (var conn = Config.DB.Open())
            {
                ...
                object result = SqlHelper.GetDB().ExecuteScalar( "SaveSavedSearch", id, username, name, gender, country,
                                                        state, zip, city, ageFrom, ageTo, photoRequired, xmlChoiceIds,
        ...
        public static void Delete(int id)
        {
            //using (var conn = Config.DB.Open())
            {
                SqlHelper.GetDB().ExecuteNonQuery( "DeleteSavedSearch", id);
            }
        }
        ...
        public void SetNextEmailDate()
        {
            //using (var conn = Config.DB.Open())
            {
                SqlHelper.GetDB().ExecuteNonQuery( "SetNextEmailDate", id, nextEmailDate.Value.AddDays(emailFrequency));
            }
        }
    }
        ...
        private static void AsyncProcessMailerQueue(object data)
        {
            ...
            try
            {
                ...
                foreach (SavedSearch savedSearchToMail in SavedSearch.GetSavedSearchesToMail())
                {
                    ...
                    try
                    {
                        recipient = Classes.User.Load(savedSearchToMail.Username);

                        if (!recipient.ReceiveEmails || recipient.Deleted)
                            continue;
                        
                        if (recipient.Email.ToLower().Contains("@ezFixUp.com"))
                            continue;
                    }
