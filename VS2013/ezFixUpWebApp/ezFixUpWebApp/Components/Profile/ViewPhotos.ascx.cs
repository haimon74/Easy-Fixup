...
namespace ezFixUp.Components.Profile
{
    public partial class ViewPhotos : UserControl
    {
        ...
        private void LoadStrings()
        {
            ...
            if (Page is ManageMemberProfile && Config.Users.EnablePhotoAlbums)
            {
            ...
            foreach (var photoNote in PhotoNote.Load(null, photoId, null))
            {
                ...
                if (photoNote.Username != null)
                {
                    ...
                    var linkHtml = Request.IsIE6() ? 
                        "<a href=\"{0}\" class=\"skinlinkcolor\" target=\"_blank\">{1}</a> (" +
                        "<a href=\"{2}\" class=\"skinlinkcolor\" target=\"_blank\">{3}</a>)" :
                        "<a href=\"{0}\" class=\"skinlinkcolor\" onmouseover=\"$('#imgPhoto').outlineUsername('{1}');\"" +
                        " onmouseout=\"$('#imgPhoto').removeOutlineUsername('{1}');\" target=\"_blank\">{1}</a> (" +
                        "<a href=\"{2}\" class=\"skinlinkcolor\" onmouseover=\"$('#imgPhoto').outlineUsername('{1}');\"" +
