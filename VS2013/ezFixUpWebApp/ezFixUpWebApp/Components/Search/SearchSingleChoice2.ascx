<%@ Control Language="c#" AutoEventWireup="True" Codebehind="SearchSingleChoice2.ascx.cs" Inherits="ezFixUp.Components.Search.SearchSingleChoice2" TargetSchema="http://schemas.microsoft.com/intellisense/ie5"%>
<div id="pnlID" enableviewstate="false" runat="server">
    <div class="expander">
        <a href="#" onclick="return false;" class="plus1">
            <asp:Literal ID="ltrName" Runat="server" />
        </a>
    </div>
    <div id="divExpandee" class="expandee" style="display: none;padding-left:15px;" runat="server">
        <asp:DropDownList id="dropValues" CssClass="dropdownlist" runat="server"></asp:DropDownList>
    </div>
    <INPUT id="hidQuestionId" type="hidden" runat="server">
</div>