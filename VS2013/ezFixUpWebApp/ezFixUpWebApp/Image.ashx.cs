...
namespace ezFixUp
{
    public class ImageHandler : ezFixUpBaseAsyncHandler
    {
            ...
            string prmCache, string prmDiskCache, string prmExactCrop, string prmDebug)
        {
            ...
            if (username != null && context.Request.Params["stack"] == "1")
            {
                ...
                {
                    ShowAvatarImage(context, Classes.User.Load(username).Gender, MaxWidth, MaxHeight);
                }
                catch (Exception err)
                {
            ...
            {
                case "-1":
                    ShowAvatarImage(context, Classes.User.eGender.Male, MaxWidth, MaxHeight);
                    return;
                case "-2":
                    ShowAvatarImage(context, Classes.User.eGender.Female, MaxWidth, MaxHeight);
                    return;
                case "-3":
                    ShowAvatarImage(context, Classes.User.eGender.Couple, MaxWidth, MaxHeight);
                    return;
            }
            ...
            try
            {
                ...
                if (Id == "session")
                {
                    //image = ((Photo) Global.GetSessionState()["temp_photo"]).Image;
                    image = Image.FromFile((string)Global.GetSessionState(context.Session.SessionID)["temp_photo_fileName"]);
                    skipCaching = true;
                    dispose = false;
                }
                else
                {
                    if (Global.GetSessionState(context.Session.SessionID).ContainsKey("temp_photos"))
                    {
                        skipCaching = true;
                        var collection = (SortedList)Global.GetSessionState(context.Session.SessionID)["temp_photos"];
                    ...
                    else
                    {
                        ...
                        if (Config.Photos.EnableExplicitPhotos &&
                            ...
                            !Global.GetSessionState(context.Session.SessionID).ContainsKey("AdminSession"))
                        {
        ...
        private static void ShowAvatarImage(HttpContext context, Classes.User.eGender gender, int MaxWidth, int MaxHeight)
        {
            switch (gender)
            {
                                    ...
                                    FaceFinderPlugin.FaceRegion faceRegion, bool disposeImage)
        {
            ...
            bool isMatchmaker = (photo != null && photo.User.InterestedIn == Classes.User.eGender.Matchmaker);
        ...
        public static string CreateMatchToImageUrl(int width, int height)
        {
            ...
            if (MatchmakerHelper.MatchToUsername.IsNullOrEmpty())
            {
                ...
                if (sessionUser == null)
                    ...
                    url = CreateImageUrl(sessionUser.Gender == Classes.User.eGender.Male ? -1 : -2, 200, 200, false, true, true);
            }
            else
            {
                var user = Classes.User.Load(MatchmakerHelper.MatchToUsername);
                ...
                if (MatchmakerHelper.MatchToFriendImageId > 0)
                    ...
                    url = CreateImageUrl(user.Gender == Classes.User.eGender.Male ? -1 : -2, 200, 200, false, true, true);
            }
            return url;
        }
