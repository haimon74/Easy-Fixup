            ...
            if (CurrentUserSession != null)
            {
                ...
                if (lastSearch != null)
                {
                    User user = (MatchmakerHelper.IsMatchmakerState && MatchmakerHelper.IsMatchToFriendAlredySelected)
                                    ? User.Load(MatchmakerHelper.MatchToUsername)
                                    : CurrentUserSession;
                        ...
                        foreach (ProfileQuestion question in questions)
                        {
                                ...
                                (CurrentUserSession != null && CurrentUserSession.Paid))
                            {
                                ...
                                if (!Config.Users.DisableGenderInformation)
                                {
                                    ...
                                    divQuestionContainer.Attributes["class"] = genderTrim();
                                    //divQuestionContainer.Style.Add("padding-left", "10px");

                                    if ((SearchGenderPicker1.LookingForGender == (int)User.eGender.Male
                                         && !question.VisibleForMale) ||
                                        (SearchGenderPicker1.LookingForGender == (int)User.eGender.Female
                                         && !question.VisibleForFemale) ||
                                        (SearchGenderPicker1.LookingForGender == (int)User.eGender.Couple
                                         && !question.VisibleForCouple))
                                    {
                ...
                if (search is CustomSearch)
                {
                    ...
                    foreach (IProfileSearchComponent searchTerm in controls)
                    {
                        if (searchTerm.Answers != null && searchTerm.Answers.Length > 0)
                        {
                            ...
                            if (!Config.Users.DisableGenderInformation &&
                                (!question.VisibleForMale && search.Gender == User.eGender.Male
                                 || !question.VisibleForFemale && search.Gender == User.eGender.Female
                                 || !question.VisibleForCouple && search.Gender == User.eGender.Couple))
                                continue;
